...
% Returns the defaults for the Spindler detection algorithm
%
% Parameters:
%
...
defaults = struct( ...
    ...
    'spindlerFilterFrequencies', ...
    getRules([1, 35], ...
    {'numeric'}, {'row', 'positive'}, ... 
    'Two element-vector with initial filtering limits.'), ...
    'spindlerMinAtomsPerSecondInterval', ...
    getRules(0.025, ...
    {'numeric'}, {'positive'}, ... 
    'Minimum length of the eligible region for finding parameters.'), ...
    'spindlerThresholds', ...
    getRules([0, power(10, -5:1:-2), 0.02:0.01:0.1, 0.2:0.1:1], ...
    {'numeric'}, {'row', 'nonnegative', '<=', 1}, ... 
    'Thresholds to test in determining the spindles.'), ...
    'spindlerGaborFrequencyResolution', ...
    getRules(0.5, ...
    {'numeric'}, {'positive'}, ... 
    'Frequency resolution for Gabor representation.'), ...
    'spindlerGaborScales', ...
