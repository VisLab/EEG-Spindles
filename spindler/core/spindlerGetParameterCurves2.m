...
spindleRatio = zeros(size(spindleMask));
topRatio = spindle75 + spindle25 - 2*spindle50;
bottomRatio = spindle75 - spindle25;
spindleRatio(~spindleMask) = topRatio(~spindleMask)./bottomRatio(~spindleMask);
...
%% 
colors = jet(numThresholds);
for k = 1:numThresholds  
    figure
    hold on
    plot(atomsPerSecond, spindle25(:, k), 'Color', 'b', 'LineWidth', 2)
    plot(atomsPerSecond, spindle50(:, k), 'Color', 'k', 'LineWidth', 2);
    plot(atomsPerSecond, spindle75(:, k), 'Color', 'g', 'LineWidth', 2);
    plot(atomsPerSecond, xTHRatioMean, 'Color', 'r', 'LineWidth', 2);
    plot(atomsPerSecond, xTHRatio);
    hold off
    legend('25%', '50%', '75%')
    xlabel('Atoms per second')
    ylabel('Spindle length(s)');
    title(['Threshold: ' num2str(baseThresholds(k))]);
end
fprintf('xxx');
%     spindleRatio = reshape(spindleRatio, numAtoms, numThresholds);
% spindle25 = reshape(spindle25, numAtoms, numThresholds);
