function [spindleCurves, warningMsgs, warningCodes] = ...
                   spindlerGetParameterCurvesNew(spindles, outDir, params)
...
%% Get the atoms per second and thresholds
    ...
    baseTitle = [theName ':Average spindle length vs atoms/second no STD'];
    theTitle = {'Average spindle length vs atoms/second no STD'; theName; ...
        ['STD range: [' num2str(lowerAtomRateRange) ',' ...
        num2str(upperAtomRateRange) '] ' ...
        ' Best atoms/sec: ' num2str(atomsPerSecond(bestEligibleAtomInd)) ...
        ' Best threshold: ' ...
    ...
    plot(atomsPerSecond, meanSpindleLenCentral, ...
        'LineWidth', 3, 'Color', theColor, 'LineStyle', '-');
    plot(atomsPerSecond, meanSpindleLen(:, minThresholdInd), ...
        'LineWidth', 2, 'Color', theColor, 'LineStyle', '--');
    plot(atomsPerSecond, meanSpindleLen(:, maxThresholdInd), ...
        'LineWidth', 2, 'Color', theColor, 'LineStyle', ':');
    yLimits = get(gca, 'YLim');
    hLine1 = line([lowerAtomRateRange, upperAtomRateRange], [0.1, 0.1]*yLimits(2), ...
        'LineWidth', 4, 'Color', [0.85, 0.85, 0.85]);
    yLimitsTemp = [meanSpindleLen(:, maxThresholdInd), meanSpindleLen(:, minThresholdInd)];
    ...
    for k = 1:length(params.figureFormats)
        thisFormat = params.figureFormats{k};
        saveas(h1Figa, [outDir filesep theName '_AverageSpindleLength.' ...
            thisFormat], thisFormat);
    end
    if params.figureClose
        close(h1Figa);
