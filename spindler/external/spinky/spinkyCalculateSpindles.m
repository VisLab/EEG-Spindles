function spinkySpindles = spinkyCalculateSpindles(oscil, thresholds,  params)
%% Calculate spinky spindles for a set of thresholds given oscil representation
    numThresholds = length(thresholds);
    spinkySpindles(numThresholds) = struct('threshold', NaN, 'spindleList', NaN);
    ...
    function spindleList = getSpindlesForThreshold(oscil, t, params)
        ...
        for j = 1:numEpochs
            [numSpindles, pos_sp] = spDetect(oscil(j, :), t, params);
            if numSpindles == 0
                continue;
            else
                y = pos_sp;
                ...
                d = find(sb==-1);
                f = find(sa==-1);
                spindleList{j} = [d(:), f(:)]./params.srate;
            end
        end
    end
end
