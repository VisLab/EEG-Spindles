...
hdr.intro = char(fread(fp, 256, 'uchar')'); 
hdr.length = round(str2double(hdr.intro(185:192)));
hdr.records = round(str2double(hdr.intro(237:244)));
hdr.duration = str2double(hdr.intro(245:252));
hdr.channels = round(str2double(hdr.intro(253:256)));
hdr.channelname = char(fread(fp,[16, hdr.channels], 'char')');
hdr.transducer = char(fread(fp,[80,hdr.channels], 'char')');
hdr.physdime = char(fread(fp,[8,hdr.channels], 'char')');
hdr.physmin = str2double(char(fread(fp,[8,hdr.channels], 'char')'));
hdr.physmax = str2double(char(fread(fp,[8,hdr.channels], 'char')'));
hdr.digimin = str2double(char(fread(fp,[8,hdr.channels], 'char')'));
hdr.digimax = str2double(char(fread(fp,[8,hdr.channels], 'char')'));
hdr.prefilt = char(fread(fp,[80,hdr.channels], 'char')');
hdr.numbersperrecord = str2double(char(fread(fp,[8,hdr.channels],'char')'));
...
try %#ok<TRYNC>
    ...
    header.hour = round(str2double(hdr.intro(177:178)));
    header.minute = round(str2double(hdr.intro(180:181)));
    header.second = round(str2double(hdr.intro(183:184)));
    header.ID = tmp{3,1};
...
header.events.DUR = [];
header.events.OFF = [];
while m > 0 && strcmp(hdr.channelname(m, 1:15), 'EDF Annotations')
...
while m > 1 && ~(sum(hdr.numbersperrecord(1:m)) == hdr.numbersperrecord(1)*m)
    header.numchannels = header.numchannels - 1;
    header.channels = header.channels(1:end-1, :);
    header.aux{auxchannel+1, 1} = ...
        reshape(data((end - hdr.numbersperrecord(m) + 1):end,:), [1 (hdr.numbersperrecord(m)*size(data,2))]);
    header.aux{auxchannel+1, 2} = ...
        hdr.numbersperrecord(m) / hdr.duration;
    data = data(1:end-hdr.numbersperrecord(m), :);
    m = m - 1;
