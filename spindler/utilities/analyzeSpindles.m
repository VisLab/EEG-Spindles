...
xMedian = median(xAll, 2);
xMad = mad(xAll, 1, 2);
xMean = mean(xAll, 2);
xStd = std(xAll, 0, 2);
[iMedianAtoms, xMedianMin] = ...
                     findMinAfterMax(atomsPerSecond', xMedian);
[iMeanAtoms, xMeanMin] = ...
                     findMinAfterMax(atomsPerSecond', xMean);
...
plot(atomsPerSecond', xMean, 'LineWidth', 3, 'Color', [0.6, 0.6, 0.6]);
plot(atomsPerSecond', xMean+xStd, 'LineWidth', 2, ...
    'Color', [0.6, 0.6, 0.6], 'LineStyle', '--');
plot(atomsPerSecond', xMean-xStd, 'LineWidth', 2, ...
    'Color', [0.6, 0.6, 0.6], 'LineStyle', '--');
plot(atomsPerSecond', xMedian, 'LineWidth', 3, 'Color', [0.75, 0.75, 0.75]);
plot(atomsPerSecond', xMedian+xMad, 'LineWidth', 2, ...
...
%% Spindle time/spindle hits versus atoms
if verbose
    theTitle = [theName ': Spindle hits/spindle time vs atoms/second'];
    h6 = figure('Name', theTitle);
    hold on
    for j = 1:numberThresholds
        plot(atomsPerSecond, spindleHits(:, j)./spindleTime(:, j), 'LineWidth', 2, ...
            'Color', theColors(j, :));
    end
    hold off
...
if verbose
    ...
    h13 = figure('Name', theTitle, 'Interpreter', 'None');
    hold on
    for j = 1:numberThresholds
