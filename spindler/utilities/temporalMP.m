function [r, atomParams, scaledB, r2Values] = temporalMP(y, B, nonnegative, maxiter, ...
    ...
    %     functions
    %     atomParams: atomIndex, timeFrame, amplitude for each iteration
    %Example use:
    ...
    C = C(:, round((size(B, 2) + 1)/2) + (0:length(y) - 1));
    
    %Precompute convolution of each basis function with the others 
    ...
    while ii <= maxiter        
        ...
        atomParams(ii, 3) = C(maxi, maxj);
        atomParams(ii, 2) = maxj;
        atomParams(ii, 1) = maxi;
...
%% Update Cmax
        ...
        if hipos*nsqrt <= length(Cmax)
            Cmax2(lopos:hipos) = max(reshape(Cmax(((lopos - 1)*nsqrt + 1) ...
                                  :(hipos*nsqrt)), nsqrt, hipos - lopos + 1));
        else
            %Pad with zeros
            At = Cmax(((lopos - 1)*nsqrt + 1):end);
            A = [At, zeros(1, (hipos - lopos + 1)*nsqrt - length(At))];
